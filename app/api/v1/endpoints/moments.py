...
def create_moment(moment: MomentCreate, current_user: crud_user.get_user = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db_moment = crud_moment.create_moment(db, moment=moment, user_id=current_user.user_id)
...
def read_all_moments(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    moments = crud_moment.get_all_moments(db, skip=skip, limit=limit)
...
def read_moment(moment_id: uuid.UUID, db: Session = Depends(get_db)):
    moment = crud_moment.get_moment(db, moment_id=moment_id)
    if not moment:
...
def update_moment(
    ...
    db_moment = crud_moment.get_moment(db, moment_id=moment_id)
    if not db_moment:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Moment not found")
    ...
    updated_moment = crud_moment.update_moment(db, db_moment=db_moment, moment_in=moment_in, user_id=current_user.user_id)
...
def delete_moment(
    ...
    crud_moment.delete_moment(db, moment_id=moment_id)
    return